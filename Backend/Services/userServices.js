 ...
 // const htmlTemplate = fs.readFileSync('otp.html', 'utf8');
  ...
  const userSignup = async (req, res) => {
    ...
    try {
      ...
      const content = `<p>Hi ${req.body.username},</p><p>Please click on the link to verify your email:</p><a href="https://localhost:3000/mail-verification?token=${randomToken}">Click here</a>`;
...
const forgetPassword = async (req, res) => {
  try {
    ...
    if (user) {
      ...
      const randomToken = randomstring.generate();
      const content = `<p>Hi ${user.username}</p><p>Please click on the link to reset your password:</p><a href="https://localhost:3000/forget-Password?token=${randomToken}">Click here</a>`;
